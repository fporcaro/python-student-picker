...
class BasketItemModelTestCase(unittest.TestCase):
    def test_basket(self):
        ...
        self.assertEqual(2, len(basket_item_model.main_items))

        item2 = basket_item_model.select_item()
        self.assertIn(item2, items)
        self.assertEqual(1, len(basket_item_model.main_items))

        item3 = basket_item_model.select_item()
        self.assertIn(item3, items)
        self.assertEqual(0, len(basket_item_model.main_items))

        basket_item_model.reset_basket()
        self.assertEqual(3, len(basket_item_model.main_items))
